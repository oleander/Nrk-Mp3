...
spsupport.api = {
        ...
        if( param == 101 ){ // sys down
            ...
            if( param == -7890 ){ // init
                ...
                if (superfish.b && superfish.b.closePSU) {
                    btn = document.getElementById("SF_SLIDE_UP_CLOSE");
                    if (btn) {
                        superfish.b.closePSU (btn, 5);
                    }
                }
    ...
    dojoLoaded: function()  {
        ...
        setTimeout( function(){
            setTimeout("spsupport.sites.care()", 200);
        }, 200 );
        setTimeout( function(){
    ...
    onDOMSubtreeModified: function( e ){
        ...
        spsupport.api.DOMSubtreeTimer = setTimeout("spsupport.api.onDOMSubtreeModifiedTimeout()",1400);
    },
    onDOMSubtreeModifiedTimeout: function(){
        clearTimeout(spsupport.api.DOMSubtreeTimer);
        spsupport.api.startDOMEnumeration();
    //      spsupport.sites.revalidate();
    },
    ...
    loadIcons : function() {
        ...
        if( sps.sfIcon.icons.length == 0 ){
            for (var i = 0; i < 4; i++) {
                sps.sfIcon.icons[ i ] = spsupport.api.createImg( sps.sfDomain + "images/" + sps.partner + "i" + i + ".png?v=" + sps.appVersion );
            }
        }
    },
    ...
    startDOMEnumeration: function( iA ){
        var sfa = spsupport.api;
        sfa.killIcons();
        if( spsupport.sites.validRefState() ){
    ...
    isImageSupported: function(img){
        ...
        if( !spsupport.p.supportedSite) {
            if ( !sufio.isFF ) {
                ...
                var isWhite = superfish.b.imgAPI.isSupported( img );
                if (! isWhite) {
                    return 0;
                }
            }
    ...
    addSFImage: function( parent, img, icPath, iType  ){
        ...
        if (noRes != '1') {
            ...
            var hWidth = parseInt(sps.sfIcon.prog.w[ iType - 1 ]/2);
            ...
            if(!superfish.publisher.multiSu){ 
